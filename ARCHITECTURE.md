...
| Hosted Agent | The top-level product surface is the versioned Foundry Hosted Agent `pathforward-orchestrator` (`agent.yaml`, `Dockerfile`, `hosted/pathforward_orchestrator/main.py`). It exposes the `responses` protocol, loads the `/pathforward` Skill from Foundry Toolbox MCP, runs the bounded Orchestrator route, and preserves the deterministic Evidence Gate / mint spine. Live proof exists on Hosted Agent version 18 (2026-06-10): the hosted route completed with live Foundry agents, Azure AI Search retrieval, Fabric-live Program Insights, mint authorization requested, authorized mint, denied authorization/no-mint, and semantic ABSTAIN/no-mint. |
| Reliability | Loop hard-capped **N=3 → fail-closed abstain**; the credential mint refuses abstained results and asserts `cited_edge_id == driving CertGap edge`. Mint/approval architecture claims are governed by `.agents/plans/000-non-negotiable-agentic-architecture-contract.md`; the current hosted proof uses local mint authorization code before delegating to `mint()`. |
...
| Eval / Safety | A **deterministic eval + red-team pack** (`pathforward/eval/`, `scripts/eval_groundedness.py`, `scripts/redteam_live.py`, `scripts/eval_orchestrator_live.py`, `scripts/eval_hosted_agent_live.py`) — pass/fail decided in code, never an LLM judge. A 22-family adversarial taxonomy hardened the trust boundary (cross-worker mint contamination, derived-not-supplied readiness, homoglyph leakage, numeric tie-back, uncorpused-skill refusal). Older loop numbers remain recorded separately; the **skill-loaded prompt-agent Orchestrator path** has its own live scorecards (2026-06-09): **16/16 grounded + spine-intact** and **0.0% ASR — 16/16 defenses held**, including reflection-channel and Orchestrator route attacks. The **Hosted Agent endpoint** has a version 18 hosted-target scorecard (2026-06-10): **4/4 groundedness/approval-hold cases passed**, **4/4 prompt-surface attacks held**, **ASR 0.0%**, and **1/1 semantic ABSTAIN case passed**. Microsoft Foundry's GroundednessEvaluator is a corroborating second opinion when enabled. Known LLM-judgment limitations are documented, not hidden. |
...
## Specialist reasoning loop (implemented inside the Hosted Orchestrator path)

The Foundry/Agent control-surface box is realized through the Hosted Orchestrator route:
**Orchestrator → Curator → Generator → Critic → Evidence Gate (deterministic notary) → Planner →
Program Insights**. Agents do the reasoning and code notarizes the trust boundary; the Critic agent
before the gate is implemented and advisory-only (see ADR 006). `run_multiagent` is no longer an
approved runtime, demo, live-smoke, fallback, or submission path; it may remain only for tests that
validate deterministic code contracts and legacy behavior.
...
## Hosted Orchestrator (mainline target, live proof landed)
...
creates a mint authorization request, and only mints when explicit runtime authorization is supplied.
Local proof is `tests/test_hosted_orchestrator.py`. Live proof is `scripts/smoke_hosted_agent_live.py`
against Hosted Agent version 18: toolbox diagnostics, default no-approval/no-mint, semantic ABSTAIN,
explicit approval mint, and explicit denied approval all passed through the dedicated hosted-agent
endpoint. Hosted-target eval proof with `scripts/eval_hosted_agent_live.py --limit 4 --attack-limit 4`
passed 4/4 hosted groundedness cases, held 4/4 prompt-surface attacks, and passed 1/1 hosted semantic
ABSTAIN case.
...
Agent Framework Workflow is not product architecture, not a dependency target, and not the next
parity item. Do not build on it unless the user explicitly re-authorizes Workflow.
...
- **Fabric Program Insights** → two live-capable routes: `FabricInsightsClient`
  ...
  `scripts/smoke_fabric_live.py` and Hosted Agent version 18 (`source="fabric-live"`).
