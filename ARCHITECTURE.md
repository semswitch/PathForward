...
approval mint all passed through the dedicated hosted-agent endpoint. Hosted-target eval proof has
started with `scripts/eval_hosted_agent_live.py --limit 3 --attack-limit 3`: 2/3 hosted groundedness
cases passed and 3/3 prompt-surface attacks held. Remaining hosted gaps are broader eval coverage,
semantic ABSTAIN proof, explicit denial UX if needed, and Fabric data-agent reliability after one
EMP-004 hosted run failed inside Fabric before returning PathForward proof JSON.
