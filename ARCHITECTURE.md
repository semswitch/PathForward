...
against Hosted Agent version 11: toolbox diagnostics, default no-approval/no-mint, and explicit
approval mint all passed through the dedicated hosted-agent endpoint. The hosted core now also has
local proof for semantic ABSTAIN and explicit denied approval, and `scripts/smoke_hosted_agent_live.py`
/ `scripts/eval_hosted_agent_live.py` are wired to capture those paths live. Hosted-target eval proof
has started with `scripts/eval_hosted_agent_live.py --limit 3 --attack-limit 3`: 2/3 hosted
groundedness cases passed and 3/3 prompt-surface attacks held. Remaining hosted gaps are broader eval
coverage plus redeploy/live proof for semantic ABSTAIN, explicit denied approval, and controlled
Fabric fallback/retry after one EMP-004 hosted run failed inside Fabric before returning PathForward
proof JSON.
