...
@dataclass
class FabricInsightsClient:
    """Drop-in LLMClient for a Foundry prompt agent with the Microsoft Fabric data agent tool.

    The model answers cohort/program questions by issuing NL2SQL queries against a PUBLISHED Fabric
    data agent over the OneLake star-schema, via `MicrosoftFabricPreviewTool`, under On-Behalf-Of
    identity (the signed-in user — service principal is NOT supported for the Fabric data agent). This
    is the live 'fabric-live' tier for ProgramInsightsAgent; it stays OFF the credential mint trust
    path (advisory), and `cohort.py` (over the single `derivation.py` source) remains the
    reconciliation anchor — a Fabric answer that diverges is flagged, never trusted to gate.

    Lazily creates ONE prompt-agent version (Fabric tool + optional caller schema) on first respond()
    and reuses it; call close() to delete it. Keyless via DefaultAzureCredential.

    Verified (Learn MCP, 2026-06-08): the prompt-agent path is PromptAgentDefinition(tools=[
    MicrosoftFabricPreviewTool(fabric_dataagent_preview=FabricDataAgentToolParameters(
    project_connections=[ToolProjectConnection(project_connection_id=<fabric connection id>)]))])
    consumed via responses.create(..., agent_reference). `connection_name` resolves to a Foundry
    'Microsoft Fabric' connection that carries the data agent's workspace_id + artifact_id; only one
    Fabric data agent per agent; same tenant.
    (learn.microsoft.com/azure/foundry/agents/how-to/tools/fabric)
    """
    endpoint: str
    connection_name: str
    agent_name: str = "pathforward-insights-fabric"
    model: str = "reasoning"
    force_tool: bool = True   # tool_choice="required" so the Fabric data agent is always consulted
    use_cli_credential: bool = True   # Fabric OBO: use the az-logged-in USER, NOT the .env service principal
    _project: object = field(default=None, repr=False)
    _openai: object = field(default=None, repr=False)
    _agent: object = field(default=None, repr=False)
    ...
    def _ensure(self, instructions: str, schema: Optional[dict]) -> None:
        ...
        from azure.ai.projects import AIProjectClient
        from azure.ai.projects.models import (
            FabricDataAgentToolParameters, MicrosoftFabricPreviewTool, PromptAgentDefinition,
            PromptAgentDefinitionTextOptions, TextResponseFormatJsonSchema, ToolProjectConnection,
        )
        # The Fabric data agent uses identity passthrough (OBO) and does NOT support service principals.
        # Use the az-logged-in USER (AzureCliCredential) so a service principal in .env's AZURE_CLIENT_*
        # is never picked up (DefaultAzureCredential would prefer the SP EnvironmentCredential). Run
        # `az login` as a user first. Set use_cli_credential=False only for a non-Fabric debug path.
        ...
        self._project = AIProjectClient(endpoint=self.endpoint, credential=cred)
        self._openai = self._project.get_openai_client()
        conn_id = self._project.connections.get(self.connection_name).id
        tool = MicrosoftFabricPreviewTool(fabric_dataagent_preview=FabricDataAgentToolParameters(
            project_connections=[ToolProjectConnection(project_connection_id=conn_id)]))
        text = None
        if schema:
            text = PromptAgentDefinitionTextOptions(format=TextResponseFormatJsonSchema(
                type="json_schema", name="insights_output", schema=schema, strict=False))
        self._agent = self._project.agents.create_version(
            agent_name=self.agent_name,
            definition=PromptAgentDefinition(model=self.model, instructions=instructions,
                                             tools=[tool], text=text),
            description="PathForward live Program Insights agent (Microsoft Fabric data agent).",
        )

    def _create_with_backoff(self, input: str):
        """Call the agent, forcing/allowing the Fabric tool; back off on the 429 rate limit."""
        last: Optional[Exception] = None
        for attempt in range(_RATE_LIMIT_RETRIES):
            try:
                return self._openai.responses.create(
    ...
    def respond(self, instructions: str, input: str, *,
        ...
        try:
            ...
            if _is_content_filter(exc):
                # RAI blocked the prompt — surface an empty parse so the caller degrades safely.
                return LLMResponse("", "", {"_content_filtered": True}, previous_response_id,
                                   retrieved_ref_ids=())
            raise
        try:
            ...
            parsed = {}
        return LLMResponse(getattr(resp, "id", ""), resp.output_text or "", parsed,
                           previous_response_id, retrieved_ref_ids=())   # NL2SQL != corpus retrieval
